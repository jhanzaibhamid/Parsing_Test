...
int main(void)
{
  ...
  else if (error == 1) printf("Header Missing\n");
  else if (error == 2) printf("Data Missing or Wrong Format\n");
  else if (error == 3) printf("Empty Data\n");
  else if (error == 10) printf("Bad Checksum\n");
  else if (error > 10) printf("Bad Checksum & Empty Data\n");
 
  
  printf("UTC = %lf\n", gga.utc);
  printf("Latitude =  %lf %c\n", gga.lat, gga.lat_dir); 
  printf("Longitude =  %lf %c\n", gga.lon, gga.lon_dir);
  printf("Altitude =  %lf %c\n", gga.alt, gga.alt_unit);
